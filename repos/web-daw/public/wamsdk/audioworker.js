  ...
  function registerOnWorker(name, ctor) {
    if (!ctors[name]) {
      ...
      postMessage({
        type: 'register',
        name: name,
        descriptor: ctor.parameterDescriptors,
      })
    } else {
...
onmessage = function (e) {
  ...
  switch (msg.type) {
    ...
    case 'createProcessor':
      ...
      // -- create processor
      var processor = AudioWorkletGlobalScope._createProcessor(
        a.name,
        e.ports[0],
        a.options
      )
    ...
    case 'process':
      ...
      if (processor) {
        if (hasSAB) {
          ...
          postMessage(
            { buf: msg.buf, type: 'process', node: processor.awp.node },
            [msg.buf]
          )
        }
      }
