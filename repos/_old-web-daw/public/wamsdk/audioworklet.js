...
(function(scope, scripts) {
  ...
  AWPF.polyfill = function (scope, urls) {

    function loadScripts (resolve) {
      if (urls) {
        function loadInOrder(i, done) {
          if (urls[i]) {
            var script = document.createElement("script");
            script.src = urls[i];
            document.head.appendChild(script);
            script.onload = function () { loadInOrder(++i, done); }
          }
          else done();
        }
        loadInOrder(0, function () { resolve(); } )
      }
      else resolve();
    }
    ...
    return new Promise( function (resolve) {
      ...
      if (AWPF.AudioWorkletAvailable(scope))
        ...
        fetch(AWPF.origin + "/public/wamsdk/audioworker.js").then(function (resp) {
